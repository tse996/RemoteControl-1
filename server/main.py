...
def handle(conn):
    ...
    while True:
        cv2.waitKey(100)
        imgnpn = np.asarray(ImageGrab.grab())
...
def info(conn1):
    ...
    """
    while True:
        cpu_usage = cpu_percent(interval=2)  # CPU使用率
        mem = virtual_memory()  # 内存信息
        mem_usage = mem.percent  # 内存使用率
        mem_total = mem.total / (1024 ** 3)  # 总内存
        data = struct.pack('>fff', cpu_usage, mem_usage, mem_total)
        conn1.sendall(data)
...
if __name__ == '__main__':
    while True:
        ...
        threading.Thread(target=info, args=(conn1,)).start()
